...
3. 生成 WebP 预览图：
   ...
   脚本会输出 `examples/preview-01.webp`、`preview-02.webp` 等文件，并自动创建或更新 `examples/examples.md`。它会补上可以可靠推断的字段，包括图片尺寸、预览图体积、原图路径和体积、提示词版本，以及可识别的 ChatGPT 文件名时间戳日期或备注。已有非空字段会保留。
...
4. 如果希望预览图显示在自动生成的 README 目录里，在该提示词的 `meta.yaml` 中加入：
